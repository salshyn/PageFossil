...
module.exports = function (background) {
            ...
            try {
                ...
                if (pageSections && pageSections[1]) {
                    ...
                    if (filesystem.folderList.length <= 2) { // empty main folder
                        ...
                        filesystem.msg = 'Set folder hash: ' + folderHash + '.';
                        filesystem.folderHash = folderHash;
    ...
    function _examineSubfolders(subfolders) {
        ...
        filesystem.numSubfolders = subfolders.length;
            ...
            if (canceled) {
                ...
                setTimeout(function() {
                    ...
                    if (localStorage.getItem(mainFolderKey)) {
                        ...
                        var text = 'Could not determine download folder.    ' +
                            config.appName + ' may not operate as expected.';
                        log.error(text);
                        filesystem.handler.message.innerHTML = text;
                    }
    ...
    function _getPath(absPath) {
        ...
        if (config.os === 'win') {
            ...
            path = pathOnly.join('\\');
            thePath = path;
            filesystem.msg = 'Set ' + config.downloads.folderName + ' to ' + thePath + '.';
        }
        else {
            parts = absPath.split('/');
            pathOnly = parts.splice(0, parts.length - 1);
    ...
    filesystem.pollMainFolder = function() {
        ...
        setTimeout(function () {
            log.debug('Force interval timeout on filesystem poll.');
    ...
    filesystem.setAssetHandler = function(handler) {
        ...
        filesystem.msg = filesystem.handler.message;
        return;
    };
