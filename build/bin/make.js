...
var exe = {
  ...
  copyImagesSkin: {
    win32: 'robocopy "' + cwd + imagesCSSSkinWin + '" "' + extensionsFolder + 
      '\\default-skin" ' + robocopyOptions + ' || cmd /c exit / b 0',
    other: 'cp -R "' + cwd + imagesCSSSkin + '" "' + extensionsFolder + '"'
  },
  copyJS: {
    win32: 'robocopy "' + cwd + jsFolderName.replace(/\//g,'\\')     + '" "' +
      extensionsFolder + '\js" '     + robocopyOptions + ' || cmd /c exit / b 0',
...
async.series([
    ...
    pkgJsonReader.on('line', function (line) {
      for (f in attributeRegexes) {
        ...
        if (matches && matches[1] && matches[2]) {
          options['___' + matches[1].toUpperCase() + '___'] = matches[2];
        }
      }
    });
  ...
  function(callback) {
    var assets = [
      ...
      { name: 'Images', cmd: exe.copyImages[platform] }
    ];
    ...
    for (var i=0; i <= assets.length - 1; i++) {
      ...
      exec(assets[i].cmd, function(status, output) {
        if (status) {
          console.log('  ERROR building ' + assets[i].name + '- Exit status:', status);
          process.exit(0);
        }
        count++;
        if (i === count)
    ...
    async.each(filenames,
      function(file, cb){
        resize(fs.readFileSync(imageLogo), {
          ...
          fs.writeFileSync(extensionsFolder + 'images/' + file + '.png', buf);
  ...
  function(callback) {
    ...
    if (buildEnv === 'dev') {
      ...
      compileEventString = '"' + browserifyBin + '" "' + cwd + sourceFolder + 
        'event.js" > "' + cwd + releaseFolder + 'event.js" ' + buildFlags;

      compilePopupString = '"' + browserifyBin + '" "' + cwd + sourceFolder + 
        'popup.js" > "' + cwd + releaseFolder + 'popup.js" ' + buildFlags;

      compileViewString = '"' + browserifyBin + '" "' + cwd + sourceFolder + 
        'view.js" > "' + cwd + releaseFolder + 'view.js" ' + buildFlags;
    }
